...
def parse_amount_smart(text: str) -> float | None:
    ...
    Supports: 5000, 5k, 50K, 5L, 5 lakh, 1 cr, 1.5cr,
              ₹5000, Rs.5000, 5,000, 50,000, 10000/-, 5000 rs
    ...
    # Strip currency symbols / suffixes noise first
    t = _re_amt.sub(r'[₹$€£¥]', '', t)          # currency symbols
    t = _re_amt.sub(r'\brs\.?\b', '', t)        # rs / rs.
    t = _re_amt.sub(r'/-$', '', t)               # trailing /-
    t = _re_amt.sub(r'\brupees?\b', '', t)      # rupee/rupees word
    t = t.replace(',', '').replace('_', '').replace(' ', '')  # commas, underscores, spaces

    # Lakh variants: 5L, 5l, 5lakh, 5 lakh, 2.5L
    ...
    # Crore variants: 1cr, 1.5cr, 1crore
