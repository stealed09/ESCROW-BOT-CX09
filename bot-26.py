...
async def cmd_instructions(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "⚠️ <i>Group auto-closes 24 hours after last action.</i>\n\n"
        ...
        "• Bot auto-checks at deal release time\n"
...
async def cmd_balance(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"🪙 Token: {token_lbl}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📥 Expected:  <b>{expected_amt} {sym}</b>\n"
...
async def partial_confirm_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_full or after_release <= 0:
        ...
        msg = (
            ...
            f"{tx_line}"
            f"\u23f0 {now_ist}\n\n"
            f"\U0001f389 Escrow completed! Group will be deleted in <b>24 hours</b>."
        )
        async def _delete_group_later():
            await asyncio.sleep(86400)
            try:
                await ctx.bot.send_message(chat_id=deal["group_id"], text="\u23f0 <b>This escrow group is now closing. Goodbye!</b>", parse_mode="HTML")
                await ctx.bot.leave_chat(deal["group_id"])
            except Exception as ex:
                logger.warning(f"Could not close group {deal['group_id']}: {ex}")
        asyncio.create_task(_delete_group_later())
    else:
        deal["status"] = "FUNDED"
        msg = (
            ...
            f"\u26a0\ufe0f Group auto-closes <b>24 hours</b> after last action."
        )
        async def _check_idle():
            await asyncio.sleep(86400)
            try:
                last_dt = datetime.fromisoformat(deal.get("last_action_at", datetime.utcnow().isoformat()))
                elapsed = (datetime.utcnow() - last_dt).total_seconds()
                if elapsed >= 86400 and deal.get("status") not in ("COMPLETED", "REFUNDED", "CANCELLED"):
                    await ctx.bot.send_message(
                        chat_id=deal["group_id"],
                        text=(
                            f"\u23f0 <b>24 hours of inactivity.</b>\n\n"
                            f"\U0001f512 Remaining <b>{after_release} {sym}</b> still in escrow.\n"
                            f"Please use /release or /refund to settle, or contact admin."
                        ),
                        parse_mode="HTML"
                    )
            except Exception as ex:
                logger.warning(f"Idle check error: {ex}")
