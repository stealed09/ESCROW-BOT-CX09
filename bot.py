...
def strip_field_value(val: str) -> str:
    """Clean up a field value — strip spaces, dashes, colons."""
    if not val:
        return ""
    val = val.strip().lstrip(":-").strip()
    return val if val and val not in ("-", "—", "nil", "none", "N/A") else ""
...
async def handle_both_agree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Agree buttons for buyer and seller ──
    agree_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ I Agree (Buyer)",  callback_data=f"ge_agree:buyer:{tid}:{buyer_uname}"),
        InlineKeyboardButton("✅ I Agree (Seller)", callback_data=f"ge_agree:seller:{tid}:{seller_uname}"),
    ]])
    ...
    agree_msg = (
        ...
        f"Dono parties neeche apna button dabao deal confirm karne ke liye 👇\n\n"
...
async def ge_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin "release" text trigger ──
    if text_lower == "release" and is_admin(user.id):
        deal = _resolve_ge_deal(update)
        if not deal:
            chat = update.effective_chat
            for d in reversed(list(_ge_deals.values())):
                if d.get("group_id") == chat.id and d.get("status") == "ACTIVE":
                    deal = d
                    break
