...
def parse_amount_smart(text: str) -> float | None:
    """Smart amount parser - supports: 5000, 5k, 1.2k
    Blocks invalid input (letters except k, symbols, extra text)
    Returns None if invalid."""
    import re as _re_amt
    text = text.strip().lower()
    
    # Only allow: digits, dot, k
    if _re_amt.search(r'[^0-9.k]', text):
        return None
...
async def cmd_ge_pay(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if fee_pct is None and bio_tag:  # Auto-determine fee
        # Check buyer bio
        if buyer_uname and buyer_uname != "—":
            try:
                buyer_chat = await ctx.bot.get_chat(f"@{buyer_uname}")
                if buyer_chat.bio and bio_tag.lower() in buyer_chat.bio.lower():
                    buyer_has_bio = True
            except:
                pass
        
        # Check seller bio
        if seller_uname and seller_uname != "—":
            try:
                seller_chat = await ctx.bot.get_chat(f"@{seller_uname}")
                if seller_chat.bio and bio_tag.lower() in seller_chat.bio.lower():
                    seller_has_bio = True
            except:
                pass
        
        # Apply fee based on bio
        if buyer_has_bio and seller_has_bio:
            fee_pct = getattr(state, "bio_discount_percent", state.fee_percent)
            bio_status = f"🏷 Bio: Both ✅ → {fee_pct}% discount fee"
        else:
            fee_pct = state.fee_percent
            bio_status = f"🏷 Bio: {'Single' if (buyer_has_bio or seller_has_bio) else 'Both'} ❌ → {fee_pct}% standard fee"
    elif fee_pct is None:
        fee_pct = state.fee_percent
...
async def cmd_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/profile - Generate visual profile card using React"""
    user = update.effective_user
    uname = (user.username or "").lower()
    
    if not uname:
        await update.message.reply_text("❌ Set Telegram username first!")
        return
    ...
    deals = stats["deals"]
    highest = stats["highest_deal"]
    
    # Calculate rank
    if vol >= 1000000:
        rank, level = "ELITE", min(10, int(vol / 1000000))
    elif vol >= 500000:
        rank, level = "PLATINUM", min(5, int((vol - 500000) / 100000) + 1)
    elif vol >= 100000:
        rank, level = "GOLD", min(5, int((vol - 100000) / 20000) + 1)
    elif vol >= 25000:
        rank, level = "SILVER", min(5, int((vol - 25000) / 15000) + 1)
    else:
        rank, level = "BRONZE", min(5, int(vol / 5000) + 1)
    
    stats["rank"] = rank
    _user_stats[uname] = stats
    
    # Create React profile card
    react_code = f'''
import React from 'react';

export default function EscrowProfile() {{
  const stats = {{
    ...
    <div className="min-h-screen bg-black text-white p-8 font-sans">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-2xl">@{{stats.username}}</p>
          <div className="flex items-center gap-2 mt-4">
            <span className="text-yellow-500">✓</span>
            <span className="text-yellow-500 text-xl">TRUSTED ESCROWER</span>
          </div>
        </div>
        
        {{/* Rank Badge */}}
        <div className="text-right">
          <p className="text-gray-400 text-xl mb-2">ESCROW RANK</p>
          <div className={{"flex items-center gap-4 bg-gradient-to-r " + rankColors[stats.rank] + " p-6 rounded-lg"}}>
            <span className="text-7xl font-bold">#{"{"}stats.level{"}"}</span>
          </div>
          <p className="text-yellow-500 text-xl mt-2">{"{"}stats.rank{"}"} ESCROWER</p>
        </div>
      </div>

      {{/* Stats Panel */}}
      <div className="border-2 border-yellow-500 rounded-lg p-8 mt-12">
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 text-sm mt-2">INR</p>
          </div>

          {{/* Max Escrow */}}
          <div className="text-center">
            <div className="text-5xl mb-4">🛡️</div>
            <p className="text-gray-400 text-sm mb-2">MAX ESCROW</p>
            <p className="text-yellow-500 text-3xl font-bold">
              ₹{"{"}stats.maxEscrow.toLocaleString('en-IN'){"}"}
            </p>
            <p className="text-gray-600 text-sm mt-2">INR</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 text-sm mt-2">DEALS</p>
          </div>

          {{/* Fastest Escrow */}}
          <div className="text-center">
            <div className="text-5xl mb-4">⚡</div>
            <p className="text-gray-400 text-sm mb-2">FASTEST ESCROW</p>
            <p className="text-yellow-500 text-3xl font-bold">00:00</p>
            <p className="text-gray-600 text-sm mt-2">HRS</p>
          </div>

          {{/* Merchant Rank */}}
          <div className="text-center">
            <div className="text-5xl mb-4">⭐</div>
            <p className="text-gray-400 text-sm mb-2">MERCHANT RANK</p>
            <p className="text-yellow-500 text-3xl font-bold">#{"{"}stats.level{"}"}</p>
            <p className="text-gray-600 text-sm mt-2">RANK</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 text-center">
        <div className="inline-flex items-center gap-3 text-gray-400 text-xl">
          <span className="text-yellow-500 text-3xl">✓</span>
          <span>@PAGALWORLD YOUR ONLY TRUSTED ESCROW SERVICE</span>
        </div>
      </div>
    </div>
  );
...
'''
    ...
    # Save as React artifact
    import os
    os.makedirs("/mnt/user-data/outputs", exist_ok=True)
    profile_path = f"/mnt/user-data/outputs/profile_{uname}.jsx"
    ...
    await update.message.reply_text(
        ...
        f"💰 ₹{vol:,.0f}\n"
        f"📈 {deals} deals\n\n"
        f"Opening visual profile...",
        parse_mode="HTML"
    )
    
    # Send as document so user can open in browser
    try:
        with open(profile_path, 'rb') as f:
            await update.message.reply_document(
                document=f,
                filename=f"escrow_profile_{user.username}.html",
                caption="Open this file in browser to view your profile card!"
            )
    except:
        pass
...
async def handle_user_callbacks(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "user:stats":
        ...
        await q.edit_message_text(
            f"📖 <b>Help & Guide</b>\n\n"
            f"<b>Commands:</b>\n"
            f"/profile — Your escrow profile\n"
            f"/calc AMOUNT — Fee calculator\n"
            f"/mystatus — Check deal status\n\n"
            f"<b>How it works:</b>\n"
            f"1. Admin locks deal in group\n"
            f"2. Both parties agree\n"
            f"3. Buyer pays admin\n"
            f"4. Admin releases to seller\n"
            f"5. Deal complete!\n\n"
            f"<b>Support:</b>\n"
