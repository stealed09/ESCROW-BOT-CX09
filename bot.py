...
def is_allowed_group(chat_id: int) -> bool:
    """Bot sirf assigned escrow group mein kaam karega. Private chat admins ke liye always allowed."""
    esc = getattr(state, "escrow_group_id", None)
    if not esc:
        return True  # group set nahi hua abhi tak, sab allow
    return chat_id == esc
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        f"<b>Welcome to BABA ESCROW Bot</b>\n\n"
    ...
    # User buttons
    # Channel button first if set
    ch_row = []
    if getattr(state, "channel_link", None):
        ch_name = getattr(state, "channel_name", "Our Channel")
        ch_row = [InlineKeyboardButton(f"📢 {ch_name}", url=state.channel_link)]

    kb_rows = []
    if ch_row:
        kb_rows.append(ch_row)
...
async def handle_admin_panel_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE, d: str):
    ...
    if action == "noop":
        ...
        if not is_main_admin(q.from_user.id):
            await q.answer("❌ Sirf main admin kar sakta hai.", show_alert=True)
            return
...
def main():
    ...
    app.add_handler(CommandHandler("profile",        cmd_myprofile_card))
    ...
    app.add_handler(CommandHandler("paylink",        cmd_paylink))
    app.add_handler(CommandHandler("adminprofile",   cmd_admin_profile))
...
_user_warns:        dict[str, int]   = {}     # username.lower() -> warn count
_user_lastseen:     dict[str, str]   = {}     # username.lower() -> last seen ISO string
_user_stats:        dict[str, dict]  = {}     # username -> {volume, deals, highest, rank}

# ── Persistence helpers ──────────────────────────────────
_STATS_FILE = "user_stats.json"

def _save_user_stats():
    try:
        with open(_STATS_FILE, "w") as _f:
            _json.dump(_user_stats, _f)
    ...
    try:
        if _os.path.exists(_STATS_FILE):
            with open(_STATS_FILE, "r") as _f:
                _user_stats = _json.load(_f)
    except Exception:
        pass

_load_user_stats()  # load on startup
...
async def cmd_ge_setgroup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Set escrow group via main admin DM only. Usage: /setescrowgroup GROUP_ID"""
    user = update.effective_user
    chat = update.effective_chat

    # Sirf main admin kar sakta hai
    if not is_main_admin(user.id):
        await update.message.reply_text("❌ Sirf main admin kar sakta hai.")
        return

    # Sirf private DM se
    if chat.type != "private":
        await update.message.reply_text(
            "❌ Ye command sirf apne DM mein karo.\nUsage: <code>/setescrowgroup GROUP_ID</code>",
            parse_mode="HTML"
        )
        return

    # Group ID args se lena hoga
    if not ctx.args:
        current = getattr(state, "escrow_group_id", None)
        cur_txt = f"<code>{current}</code>" if current else "❌ Set nahi hai"
        await update.message.reply_text(
            f"ℹ️ <b>Current Escrow Group:</b> {cur_txt}\n\nUsage: <code>/setescrowgroup -100XXXXXXXXXX</code>\nGroup ID pane ke liye @userinfobot se check karo.",
            parse_mode="HTML"
        )
        return

    try:
        new_gid = int(ctx.args[0])
    except ValueError:
        await update.message.reply_text(
            "❌ Valid Group ID do.\nExample: <code>/setescrowgroup -1001234567890</code>",
            parse_mode="HTML"
        )
        return

    # Set karo
    state.escrow_group_id = new_gid
    if new_gid not in _ge_config:
        _ge_config[new_gid] = {"upi": {}, "log_group_id": state.log_group_id}
    else:
        _ge_config[new_gid]["log_group_id"] = state.log_group_id

    await update.message.reply_text(
        f"✅ <b>Escrow Group Set!</b>\n\n🆔 <code>{new_gid}</code>\n\nAb bot sirf is group mein kaam karega.",
        parse_mode="HTML"
    )
...
async def cmd_ge_pay(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    # Group restriction — only work in assigned escrow group
    if update.effective_chat.type in ("group","supergroup") and not is_allowed_group(update.effective_chat.id):
        return
    chat = update.effective_chat
    if not is_admin(user.id):
        await update.message.reply_text("❌ Admin only.")
...
async def cmd_paylink(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/paylink GE-XXXX — Reply to payment message with Trade ID; sends form+payment links to vouch+log group."""
    user = update.effective_user
    # Group restriction — only work in assigned escrow group
    if update.effective_chat.type in ("group","supergroup") and not is_allowed_group(update.effective_chat.id):
        return
    ...
    # Must be a reply to the payment message
    replied = msg.reply_to_message
    if not replied:
        await msg.reply_text(
            "❌ Payment message pe reply karke bhejo.\n"
            "Usage: <code>/paylink GE-XXXXXXXX</code>",
            parse_mode="HTML"
        )
        return
    ...
    deal = _ge_deals.get(tid_arg)
    if not deal:
        await msg.reply_text(f"❌ Deal <code>{tid_arg}</code> nahi mila.", parse_mode="HTML")
        return

    # Payment link from replied message
    chat = update.effective_chat
    group_id_str = str(chat.id)
    group_short  = group_id_str[4:] if group_id_str.startswith("-100") else group_id_str
    pay_msg_id   = replied.message_id
    pay_link     = f"https://t.me/c/{group_short}/{pay_msg_id}"

    # Form link from deal
    form_msg_id = deal.get("form_message_id")
    deal_group  = str(deal.get("group_id", group_id_str))
    dg_short    = deal_group[4:] if deal_group.startswith("-100") else deal_group
    form_link   = f"https://t.me/c/{dg_short}/{form_msg_id}" if form_msg_id else "N/A"

    escrower = deal.get("escrower_username", "—")
    amount   = deal.get("received_amount") or deal.get("amount", "—")

    link_text = (
        f"<b>Form Link:-</b> {form_link}\n"
        f"<b>Payment Link:-</b> {pay_link}\n"
        f"<b>Deal Ammount:-</b> {amount}\n"
        f"<b>ESCROWER:-</b> @{escrower}"
    )
    ...
    if state.log_group_id:
        try:
            ...
            sent_to.append("Log Group")
        except Exception:
            pass

    if sent_to:
        await msg.reply_text(f"✅ Links bhej diye: {', '.join(sent_to)}")
    else:
        await msg.reply_text(
            f"⚠️ Vouch/Log group set nahi hai.\n\n{link_text}",
            parse_mode="HTML",
            disable_web_page_preview=True
        )
...
async def cmd_ge_close(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Update user stats ──
    escrower_uname = deal.get("escrower_username", "") or ""
    for uname in [buyer_uname, seller_uname, escrower_uname]:
        if uname and uname not in ("—", ""):
            ukey = uname.lower()
    ...
    _save_user_stats()  # persist to disk
...
async def cmd_calc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Support 4k=4000, 1.5L=150000, 10K, 2l etc
        amt_clean = amount_str.strip().upper()
        if amt_clean.endswith("L"):
            amount = float(_re2.sub(r"[^\d.]","",amt_clean)) * 100000
        elif amt_clean.endswith("K"):
            amount = float(_re2.sub(r"[^\d.]","",amt_clean)) * 1000
        else:
            amount = float(_re2.sub(r"[^\d.]","",amt_clean))
        if amount <= 0:
            raise ValueError
    except Exception:
        await update.message.reply_text(
            "❌ Valid amount do.\n"
            "Example: <code>/calc 5000</code> | <code>/calc 4k</code> | <code>/calc 1.5L</code>",
            parse_mode="HTML"
        )
        return
...
async def _generate_admin_profile_card(
    ...
    W, H = 1280, 720
    BG    = (10, 11, 18); CARD  = (16, 17, 28); PANEL = (20, 22, 36)
    DLINE = (35, 38, 65); BLUE  = (60, 100, 255); BLUE2 = (100, 150, 255)
    PURP  = (120, 80, 255); PURP2 = (160, 110, 255)
    WHITE = (255, 255, 255); GRAY  = (130, 135, 160); LGRAY = (55, 60, 85)

    def _lf(bold=True, size=20):
        import os
        # Poppins first (best for gaming/tech cards), then fallbacks
        cb = [
    ...
    f80=_lf(True,80); f64=_lf(True,64); f52=_lf(True,52)
    f44=_lf(True,44); f36=_lf(True,36); f28=_lf(True,28)
    f24=_lf(True,24); f22=_lf(False,22); f20=_lf(False,20)
    f18=_lf(False,18); f16=_lf(False,16)
    ...
    # Outer border
    draw.rounded_rectangle([(14,14),(W-14,H-14)], radius=22,
        fill=(*CARD,255), outline=(*PURP2,160), width=2)

    # Left accent bar
    draw.rounded_rectangle([(14,14),(24,H-14)], radius=6, fill=BLUE)
    for i in range(16):
        draw.line([(24+i,14),(24+i,H-14)], fill=(*BLUE,int(55*(1-i/16))))

    # ── HEADER ──
    AX, AY, AR = 118, 104, 72

    # Avatar
    draw.ellipse([(AX-AR-3,AY-AR-3),(AX+AR+3,AY+AR+3)], outline=(*BLUE2,255), width=3)
    draw.ellipse([(AX-AR,AY-AR),(AX+AR,AY+AR)], fill=(14,15,26))
    ...
    if pfp_bytes:
        try:
            ...
            pass
    if not pfp_placed:
        let = (first_name[0] if first_name else "?").upper()
        lw  = int(draw.textlength(let, font=f64))
        draw.text((AX-lw//2, AY-36), let, fill=BLUE2, font=f64)

    # ADMIN badge on avatar bottom
    badge_txt = "ADMIN"
    bw = int(draw.textlength(badge_txt, font=f16))
    bpad = 10
    bx1b = AX - bw//2 - bpad; bx2b = AX + bw//2 + bpad
    by1b = AY + AR - 10;       by2b = by1b + 26
    draw.rounded_rectangle([(bx1b,by1b),(bx2b,by2b)], radius=8, fill=BLUE)
    draw.text((bx1b+bpad, by1b+4), badge_txt, fill=WHITE, font=f16)

    # Name + username
    name_disp = (first_name or "Admin").upper()[:14]
    draw.text((218, AY-52), name_disp,       fill=WHITE, font=f64)
    draw.text((222, AY+14), f"@{username}",  fill=GRAY,  font=f22)

    # Rank badge top-right
    RBW, RBH = 180, 116
    rx1 = W - PAD - RBW; ry1 = 24
    draw.rounded_rectangle([(rx1,ry1),(rx1+RBW,ry1+RBH)], radius=14,
        fill=(*PANEL,255), outline=(*PURP2,200), width=2)
    draw.text((rx1+16, ry1+10), str(level), fill=BLUE2, font=f80)
    dvx = rx1 + 70
    draw.line([(dvx,ry1+18),(dvx,ry1+RBH-18)], fill=(*DLINE,255), width=2)
    def _ordsfx(n):
        s={1:"ST",2:"ND",3:"RD"}; return s.get(n if n<20 else n%10,"TH")
    draw.text((dvx+14, ry1+20), _ordsfx(level), fill=WHITE, font=f28)
    draw.text((dvx+14, ry1+62), "RANK",          fill=GRAY,  font=f22)

    # Divider
    draw.line([(PAD,192),(W-PAD,192)], fill=(*DLINE,255), width=1)

    # ── STAT BOXES ──
    BY1,BY2 = 208,372; BGAP=14
    BW = (W - PAD*2 - BGAP*2) // 3
    labels = ["TOTAL VOLUME","TOTAL DEALS","HIGHEST DEAL"]
    values = [f"Rs.{vol:,.0f}", str(deals), f"Rs.{highest:,.0f}"]
    units  = ["INR","DEALS","INR"]
    icons  = ["$","#","^"]

    for i in range(3):
        bx1 = PAD + i*(BW+BGAP); bx2 = bx1+BW
        draw.rounded_rectangle([(bx1,BY1),(bx2,BY2)], radius=14,
            fill=(*PANEL,255), outline=(*DLINE,255), width=1)
        ic_cx=bx1+34; ic_cy=BY1+34; ic_r=20
        draw.ellipse([(ic_cx-ic_r,ic_cy-ic_r),(ic_cx+ic_r,ic_cy+ic_r)],
            fill=(18,22,48), outline=(*BLUE,160), width=2)
        iw=int(draw.textlength(icons[i],font=f18))
        draw.text((ic_cx-iw//2,ic_cy-10), icons[i], fill=BLUE2, font=f18)
        draw.text((bx1+64,BY1+20), labels[i], fill=GRAY, font=f18)
        vf=f52 if draw.textlength(values[i],font=f52)<BW-32 else f44
        draw.text((bx1+18,BY1+68), values[i], fill=WHITE, font=vf)
        uw=int(draw.textlength(units[i],font=f16)); uy=BY2-30
        ll=(BW-uw-48)//2
        draw.line([(bx1+18,uy+9),(bx1+18+ll,uy+9)], fill=(*BLUE,150), width=1)
        draw.text((bx1+18+ll+8,uy), units[i], fill=BLUE2, font=f16)
        draw.line([(bx1+18+ll+8+uw+8,uy+9),(bx2-18,uy+9)], fill=(*BLUE,150), width=1)

    # ── RANK PROGRESS ──
    RP1,RP2 = 386,476
    draw.rounded_rectangle([(PAD,RP1),(W-PAD,RP2)], radius=14,
        fill=(*PANEL,255), outline=(*DLINE,255), width=1)
    draw.text((IX1,RP1+14), "RANK PROGRESS", fill=WHITE, font=f22)

    RANK_PREV={"BRONZE":0,"SILVER":25000,"GOLD":100000,"PLATINUM":500000,"ELITE":1000000}
    RANK_NEXT={"BRONZE":("SILVER",25000),"SILVER":("GOLD",100000),"GOLD":("PLATINUM",500000),"PLATINUM":("ELITE",1000000),"ELITE":("ELITE",9999999)}
    pv = RANK_PREV.get(rank,0); nr,nv = RANK_NEXT.get(rank,("ELITE",9999999))
    prog = min(1.0,(vol-pv)/max(1,nv-pv)) if rank!="ELITE" else 1.0

    pts_txt = f"Rs.{vol:,.0f} / Rs.{nv:,.0f}"
    draw.text((IX1,RP1+42), pts_txt, fill=(*BLUE2,180), font=f16)

    nxt_txt = f"NEXT RANK: {nr}"
    nw = int(draw.textlength(nxt_txt,font=f18))
    draw.text((IX2-nw,RP1+14), nxt_txt, fill=GRAY, font=f18)
    pct_txt = f"{int(prog*100)}%"
    pw = int(draw.textlength(pct_txt,font=f28))
    draw.text((IX2-pw,RP1+36), pct_txt, fill=BLUE2, font=f28)

    BAR_Y=RP1+72; BAR_H=22
    draw.rounded_rectangle([(IX1,BAR_Y),(IX2,BAR_Y+BAR_H)], radius=11, fill=(14,15,26))
    fw=max(16,int((IX2-IX1)*prog))
    bar_img=Image.new("RGBA",(fw,BAR_H),0)
    bar_d=ImageDraw.Draw(bar_img)
    for xi in range(fw):
        ratio=xi/max(fw-1,1)
        r=int(BLUE[0]*(1-ratio)+PURP[0]*ratio)
        g=int(BLUE[1]*(1-ratio)+PURP[1]*ratio)
        b=int(BLUE[2]*(1-ratio)+PURP[2]*ratio)
        stripe=(xi%18)<12
        bar_d.line([(xi,0),(xi,BAR_H)],fill=(r,g,b,255 if stripe else 190))
    mask=Image.new("L",(fw,BAR_H),0)
    ImageDraw.Draw(mask).rounded_rectangle([(0,0),(fw,BAR_H)],radius=11,fill=255)
    bar_img.putalpha(mask)
    img.paste(bar_img,(IX1,BAR_Y),bar_img)

    # ── TRUST SCORE ──
    TS1,TS2 = 490,696
    draw.rounded_rectangle([(PAD,TS1),(W-PAD,TS2)], radius=14,
        fill=(*PANEL,255), outline=(*DLINE,255), width=1)

    # Label top-left
    draw.text((IX1,TS1+14), "TRUST SCORE", fill=WHITE, font=f22)

    # Score top-right: "9.2" + "/10" same row, "EXCELLENT" below — all right-aligned
    trust = min(100, deals*7+(15 if vol>10000 else 0)+(10 if highest>5000 else 0))
    score_val = f"{min(10.0, trust/10):.1f}"
    grade = "EXCELLENT" if trust>=80 else "GOOD" if trust>=50 else "FAIR"

    sw   = int(draw.textlength(score_val, font=f52))
    tenw = int(draw.textlength("/10",     font=f20))
    excw = int(draw.textlength(grade,     font=f18))

    ten_x   = IX2 - tenw
    score_x = ten_x - sw - 4
    draw.text((score_x, TS1+10), score_val, fill=BLUE2, font=f52)
    draw.text((ten_x,   TS1+28), "/10",      fill=GRAY,  font=f20)
    draw.text((IX2-excw,TS1+64), grade,      fill=BLUE2, font=f18)

    # Bars — start BELOW score block (TS1+90), fill to TS2-16
    BARS_Y = TS1+92
    BARS   = 20
    FILLED = int(BARS * (trust/100))
    BARS_H = TS2 - BARS_Y - 16
    EACH_W = (IX2 - IX1 - (BARS-1)*5) // BARS
    ...
    for i in range(BARS):
        ...
        if i < FILLED:
            ...
            fc=(*LGRAY,200)
        draw.rounded_rectangle([(bxb,BARS_Y),(bxb+EACH_W,BARS_Y+BARS_H)], radius=5, fill=fc)
...
async def _generate_profile_card(
    ...
    W,H=1280,720
    BG=(10,11,18); CARD=(16,18,30); WHITE=(255,255,255); GRAY=(130,135,160); LGRAY=(80,85,110)
    DARK=(24,26,42); ACCENT=(60,140,255); ACCENT2=(100,180,255); GREEN=(50,215,120); GOLD=(255,185,0)
    RANK_COLORS={"BRONZE":(180,110,60),"SILVER":(180,190,210),"GOLD":(255,185,0),"PLATINUM":(100,210,255),"ELITE":(180,100,255)}
    RANK_NEXT={"BRONZE":("SILVER",25000),"SILVER":("GOLD",100000),"GOLD":("PLATINUM",500000),"PLATINUM":("ELITE",1000000),"ELITE":("ELITE",9999999)}
    RANK_PREV={"BRONZE":0,"SILVER":25000,"GOLD":100000,"PLATINUM":500000,"ELITE":1000000}
    ...
    def _lf(bold=True,size=20):
        import os
        cb=["/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf","/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf","/usr/share/fonts/truetype/freefont/FreeSansBold.ttf","/usr/share/fonts/truetype/ubuntu/Ubuntu-B.ttf","/usr/local/share/fonts/DejaVuSans-Bold.ttf"]
        cr=["/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf","/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf","/usr/share/fonts/truetype/freefont/FreeSans.ttf","/usr/share/fonts/truetype/ubuntu/Ubuntu-R.ttf","/usr/local/share/fonts/DejaVuSans.ttf"]
        for p in (cb if bold else cr):
            try:
                if os.path.exists(p): return ImageFont.truetype(p,size)
            except Exception: pass
    ...
    f72=_lf(True,72);f56=_lf(True,56);f44=_lf(True,44);f36=_lf(True,36)
    f32=_lf(True,32);f28=_lf(True,28);f24=_lf(False,24);f20=_lf(False,20)
    f18=_lf(False,18);f16=_lf(False,16)

    rank_col=RANK_COLORS.get(rank,ACCENT)

    for x in range(0,W,40):
        for y in range(0,H,40): draw.ellipse([(x-1,y-1),(x+1,y+1)],fill=(255,255,255,12))
    for r in range(300,0,-12): draw.ellipse([(W-180-r,-r+80),(W-180+r,r+80)],fill=(*ACCENT,int(18*(1-r/300))))
    for r in range(220,0,-10): draw.ellipse([(-r+100,H-80-r),(r+100,H-80+r)],fill=(*rank_col,int(14*(1-r/220))))
    draw.rounded_rectangle([(28,28),(W-28,H-28)],radius=28,fill=(*CARD,255),outline=(*ACCENT,120),width=2)
    draw.rounded_rectangle([(28,28),(36,H-28)],radius=5,fill=(*ACCENT,220))
    for i in range(16): draw.line([(56,28+i),(W-56,28+i)],fill=(*ACCENT,int(60*(1-i/16))))

    AX,AY,AR=120,144,76
    draw.ellipse([(AX-AR-3,AY-AR-3),(AX+AR+3,AY+AR+3)],outline=ACCENT,width=2)
    ...
    if not pfp_placed:
        ...
        lw=int(draw.textlength(letter,font=f56))
        draw.text((AX-lw//2,AY-28),letter,fill=ACCENT,font=f56)

    draw.text((230,76),first_name.upper()[:16],fill=WHITE,font=f72)
    draw.text((234,172),f"@{username}",fill=GRAY,font=f28)

    def ordinal(n):
        s={1:"ST",2:"ND",3:"RD"}
        return f"{n}{s.get(n if n<20 else n%10,'TH')}"
    badge_text=f"{ordinal(level)} RANK"
    btw=int(draw.textlength(badge_text,font=f28)); pad=20
    bx1=W-56-btw-pad*2; by1,by2=44,104
    draw.rounded_rectangle([(bx1,by1),(W-56,by2)],radius=12,fill=GOLD)
    tw=int(draw.textlength(badge_text,font=f28))
    cx=bx1+((W-56-bx1)-tw)//2; cy=by1+((by2-by1)-28)//2
    draw.text((cx,cy),badge_text,fill=WHITE,font=f28)

    draw.line([(60,244),(W-60,244)],fill=(*ACCENT,40),width=1)

    stats_d=[
        ("TOTAL VOLUME", f"Rs.{vol:,.0f}",    "INR"),
        ("DEALS DONE",   str(deals),           "COMPLETED"),
        ("HIGHEST DEAL", f"Rs.{highest:,.0f}", "SINGLE"),
    ]
    box_w=(W-60*2-28)//3; box_y1,box_y2=260,440
    for i,(lbl,val,unit) in enumerate(stats_d):
        bx=60+i*(box_w+14)
        draw.rounded_rectangle([(bx,box_y1),(bx+box_w,box_y2)],radius=18,fill=(22,24,40,255),outline=(*ACCENT,40),width=1)
        draw.rounded_rectangle([(bx,box_y1),(bx+box_w,box_y1+4)],radius=2,fill=(*ACCENT,120))
        draw.text((bx+18,box_y1+16),lbl,fill=LGRAY,font=f18)
        fv=f44 if draw.textlength(val,font=f44)<box_w-36 else f36
        val_y=box_y1+(box_y2-box_y1)//2-24
        draw.text((bx+18,val_y),val,fill=ACCENT2,font=fv)
        draw.text((bx+18,box_y2-28),unit,fill=LGRAY,font=f18)

    nr,nv=RANK_NEXT.get(rank,("ELITE",9999999)); pv=RANK_PREV.get(rank,0)
    prog=min(1.0,(vol-pv)/max(1,nv-pv)) if rank!="ELITE" else 1.0
    bar_x1,bar_x2,bar_y=60,W-60,466
    draw.text((bar_x1,bar_y),"RANK PROGRESS",fill=LGRAY,font=f18); bar_y+=26
    draw.rounded_rectangle([(bar_x1,bar_y),(bar_x2,bar_y+16)],radius=8,fill=(36,38,58))
    fw=max(16,int((bar_x2-bar_x1)*prog))
    draw.rounded_rectangle([(bar_x1,bar_y),(bar_x1+fw,bar_y+16)],radius=8,fill=ACCENT)
    for r2 in range(12,0,-3):
        draw.ellipse([(bar_x1+fw-r2,bar_y+8-r2),(bar_x1+fw+r2,bar_y+8+r2)],fill=(*ACCENT2,int(60*(1-r2/12))))
    draw.text((bar_x1,bar_y+22),rank,fill=rank_col,font=f18)
    if rank!="ELITE":
        nrc=RANK_COLORS.get(nr,GRAY); nrw=draw.textlength(nr,font=f18)
        draw.text((bar_x2-nrw,bar_y+22),nr,fill=nrc,font=f18)
        pt=f"{prog*100:.0f}%"; pw=draw.textlength(pt,font=f18)
        draw.text(((bar_x1+bar_x2-pw)//2,bar_y+22),pt,fill=GRAY,font=f18)

    draw.line([(60,554),(W-60,554)],fill=(255,255,255,15),width=1)
    trust=min(100,deals*7+(15 if vol>10000 else 0)+(10 if highest>5000 else 0))
    ts_col=GREEN if trust>=70 else ACCENT if trust>=40 else (220,80,80)
    draw.text((60,564),"TRUST SCORE",fill=LGRAY,font=f18)
    for i in range(10):
        bx3=60+i*42
        draw.rounded_rectangle([(bx3,590),(bx3+36,606)],radius=4,fill=ts_col if i<trust//10 else (36,38,58))
    draw.text((60,614),f"{trust}/100",fill=ts_col,font=f20)

    brand="BABA ESCROW"; bw3=draw.textlength(brand,font=f24)
    draw.text((W-60-bw3,648),brand,fill=LGRAY,font=f24)
    wm="SECURED + TRUSTED"; wmw=draw.textlength(wm,font=f18)
    draw.text(((W-wmw)//2,652),wm,fill=(*ACCENT,80),font=f18)

    buf=_io.BytesIO(); img.convert("RGB").save(buf,"PNG",optimize=True); buf.seek(0)
    return buf.getvalue()


async def cmd_myprofile_card(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/myprofile - Generate premium profile card"""
    user  = update.effective_user
    uname = (user.username or "").lower()
...
async def cmd_admin_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/adminprofile — Generate admin profile card"""
    user  = update.effective_user
    if not is_admin(user.id):
        await update.message.reply_text("❌ Admin only.")
        return
    ...
    try:
        ...
        logger.warning(f"Admin PFP fetch failed: {e}")

    await update.message.reply_chat_action("upload_photo")
    try:
        card = await _generate_admin_profile_card(
        ...
        await update.message.reply_photo(
            photo=card_buf,
            caption=(
                f"🛡 <b>{user.first_name}'s Admin Profile</b>\n\n"
                f"🏆 Rank: <b>{rank} LVL {level}</b>\n"
                f"💰 Volume: <b>Rs.{vol:,.0f}</b>\n"
                f"📦 Deals: <b>{deals}</b>\n"
                f"📈 Highest: <b>Rs.{highest:,.0f}</b>"
            ),
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Admin profile card error: {e}")
        await update.message.reply_text(f"❌ Admin profile generate nahi hua: {e}")

async def cmd_myprofile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/myprofile - Generate own profile card"""
    await cmd_myprofile_card(update, ctx)
