...
async def handle_new_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Send welcome when someone joins any group where bot is active."""
    chat    = update.effective_chat
...
async def cmd_ge_add(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/add AMOUNT — Confirm received amount and create deal record."""
    user = update.effective_user
    chat = update.effective_chat
    if not is_admin(user.id):
        await update.message.reply_text("❌ Admin only.")
    ...
    if not deal:
        await update.message.reply_text(
            ...
            "Usage: <code>/add AMOUNT</code>", parse_mode="HTML"
        )
        return
    ...
    if not ctx.args:
        await update.message.reply_text("Usage: <code>/add AMOUNT</code>", parse_mode="HTML")
        return

    try:
        recv_amt = float(_re.sub(r"[^\d.]", "", ctx.args[0]))
    except Exception:
        await update.message.reply_text("❌ Invalid amount. Example: <code>/add 500</code>", parse_mode="HTML")
        return
...
async def cmd_ge_close(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        await update.message.reply_text("Usage: <code>/close AMOUNT</code>", parse_mode="HTML")
        return

    try:
        release_amt = float(_re.sub(r"[^\d.]", "", ctx.args[0]))
    except Exception:
