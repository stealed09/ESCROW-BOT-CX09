...
async def group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id in _address_waiting:
        ...
        if other_addr and other_addr.strip().lower() == address.strip().lower():
            same_addr_warn = (
                "\n\n⚠️ <b>WARNING:</b> Buyer and Seller have the <b>same wallet address!</b>\n"
                "This is unusual. Make sure both parties have entered the correct addresses.\n"
                "If this is a mistake, <b>deal must be cancelled</b> — addresses cannot be changed."
            )
...
async def release_deal(ctx, did, deal, group_id):
    ...
    if state.required_bio:
        try:
            ...
            if state.required_bio.lower() in bio.lower():
                ...
                discount_reason  = f"Bio contains «{state.required_bio}»"
        except Exception:
            pass

    qty        = float(deal.get("quantity", 0))
    fee_pct    = state.fee_percent
    fee_amt    = qty * (fee_pct / 100) if not discount_applied else 0.0
    final      = qty - fee_amt
    ...
    if discount_applied:
        fee_line = (
            f"💸 Fee: ~~{fee_pct}%~~ → <b>0% (Discount Applied ✅)</b>\n"
            f"🏷 Reason: {discount_reason}\n"
            f"💰 Fee Saved: <b>{qty * (fee_pct / 100):.6f} {sym}</b>"
        )
    else:
        fee_line = (
...
async def cmd_refund(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin initiates refund — requires BOTH buyer and seller to confirm via buttons.
    On confirmation, fee is deducted and remaining funds returned to SELLER (who deposited).
    """
    if not is_admin(update.effective_user.id):
        return
    if not ctx.args:
    ...
    # Must be funded to have a real refund; if not funded, just cancel
    if not deal.get("funded"):
        user = update.effective_user
    ...
    deal["refunded_by"]      = update.effective_user.username or str(update.effective_user.id)
    ...
    try:
        await ctx.bot.send_message(
            chat_id=deal["group_id"],
            text=(
                f"💸 <b>REFUND REQUESTED BY ADMIN</b>\n\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"🆔 Trade ID: <code>{did}</code>\n"
