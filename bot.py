...
async def _ge_handle_release(update: Update, ctx: ContextTypes.DEFAULT_TYPE, deal: dict):
    ...
    await ctx.bot.send_message(
        ...
        text=(
            ...
            f"Apna button dabao confirm karne ke liye 👇\n"
            f"Issue hai to admin se directly baat karo."
        ),
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def ge_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── "agree" text se bhi agree ho jaye ──
    if text_lower == "agree":
        uname = (user.username or "").lower()
        # Find latest active deal in this group where user is buyer or seller
        matched_deal = None
        matched_role = None
        for d in reversed(list(_ge_deals.values())):
        ...
        if matched_deal:
            ...
            await msg.reply_text(
                "❌ Koi pending deal nahi mili tumhare liye.\n"
                "Pehle admin se deal lock karwao.",
                parse_mode="HTML"
            )
        return
    ...
    # ── Form detection — BOT validates FIRST before admin can agree ──
    form_text = text
    # Also check reply chain
    ...
    if detect_form_type(form_text):
        ...
        # ── All fields ok — notify admin to lock ──
        b_tag = f"@{buyer_raw.lstrip('@')}"
        s_tag = f"@{seller_raw.lstrip('@')}"
        await msg.reply_text(
            f"✅ <b>Form Valid!</b>\n"
            f"🛒 {b_tag} · 🏪 {s_tag} · 💰 {form_data.get('amount','—')}\n\n"
            f"Admin <b>BOTH AGREE</b> reply karke deal lock kare.",
            parse_mode="HTML"
        )
        return
