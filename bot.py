...
async def cmd_paylink(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/paylink — Admin replies to a payment message; bot sends form+payment links to vouch+log group."""
    user = update.effective_user
    msg  = update.message
    ...
    if not replied:
        await msg.reply_text(
            "❌ Kisi message pe reply karke /paylink bhejo.\n"
            "Jis message ka payment link banna hai us pe reply karo.",
            parse_mode="HTML"
        )
        return

    # Build payment link from replied message
    chat = update.effective_chat
    ...
    # Try to get deal from context (reply chain or latest deal in group)
    deal = _resolve_ge_deal(update)
    ...
    if deal:
        ...
        deal_group  = str(deal.get("group_id", ""))
    ...
    if sent_to:
        ...
        await msg.reply_text(
            f"⚠️ Links generate ho gaye but vouch/log group set nahi hai.\n\n{link_text}",
            disable_web_page_preview=True
        )
