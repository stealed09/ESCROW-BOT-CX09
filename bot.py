...
async def handle_new_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Send welcome + mute if no username."""
    chat    = update.effective_chat
    ...
    # GE group check
    is_ge_group = (state.escrow_group_id and chat.id == state.escrow_group_id) or \
                  (chat.id in _ge_config)
    # P2P deal group check
    is_p2p_group = bool(deal_by_group(chat.id)[1])

    if not is_ge_group and not is_p2p_group:
        return
...
_ge_group_latest_form: dict[int, dict] = {}  # group_id -> {text, message_id} latest unprocessed form
...
async def handle_both_agree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lock_msg = (
        f"🔒 <b>DEAL LOCKED</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"{type_emoji} Deal Type: <b>{type_label}</b>\n"
        f"🆔 Trade ID: <code>{tid}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━\n\n"
        f"💰 Amount: <b>{amt}</b>\n"
        f"🛒 Buyer: <b>{buyer}</b>\n"
    ...
    lock_msg += (
        ...
        f"<code>/pay {tid} UPI_NAME</code> — Payment QR bhejo\n"
        f"<code>/pay {tid} 2 UPI_NAME</code> — 2% fee ke saath\n"
        f"<code>/add {tid} AMOUNT</code> — Amount received confirm\n"
        f"<code>/close {tid} AMOUNT</code> — Deal complete karo\n"
        f"<code>/transfer {tid} @admin</code> — Deal transfer karo\n\n"
        f"<b>Ya phir sirf reply karke command chalao is message pe.</b>\n\n"
        f"💬 <code>release</code> likhne se buyer/seller se confirm liya jayega."
    )

    sent = await msg.reply_text(lock_msg, parse_mode="HTML")
...
async def _ge_handle_release(update: Update, ctx: ContextTypes.DEFAULT_TYPE, deal: dict):
    ...
    # Extract usernames for tagging
    buyer_tag  = f"@{buyer.lstrip('@')}"  if buyer  != "—" else "Buyer"
    seller_tag = f"@{seller.lstrip('@')}" if seller != "—" else "Seller"

    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ Confirm Release", callback_data=f"ge_confirm_release:{tid}:{user.id}")
    ]])
    ...
    await update.message.reply_text(
        ...
        f"Admin release karne wala hai.\n"
        f"<b>Neeche button dabao confirm karne ke liye.</b>\n\n"
        f"⚠️ Confirm karne ke baad paise release ho jayenge!",
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def ge_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Sirf set escrow group mein kaam karo ──
    if state.escrow_group_id and chat.id != state.escrow_group_id:
        return
    # Agar _ge_config mein bhi nahi hai to ignore
    if not state.escrow_group_id and chat.id not in _ge_config:
        return
