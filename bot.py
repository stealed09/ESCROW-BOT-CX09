...
async def _generate_profile_card(
    ...
    try:
        ...
        f36=ImageFont.truetype(fb,36); f32=ImageFont.truetype(fb,32)
        f28=ImageFont.truetype(fb,28); f22=ImageFont.truetype(fb,22)
        f18=ImageFont.truetype(fn,18); f13=ImageFont.truetype(fn,13)
    except Exception:
        f56=f44=f36=f32=f28=f22=f18=f13=ImageFont.load_default()

    rank_col=RANK_COLORS.get(rank,ACCENT)

    for x in range(0,W,36):
        for y in range(0,H,36): draw.ellipse([(x-1,y-1),(x+1,y+1)],fill=(255,255,255,12))
    for r in range(220,0,-10): draw.ellipse([(W-140-r,-r+60),(W-140+r,r+60)],fill=(*ACCENT,int(18*(1-r/220))))
    for r in range(160,0,-8): draw.ellipse([(-r+80,H-60-r),(r+80,H-60+r)],fill=(*rank_col,int(14*(1-r/160))))
    draw.rounded_rectangle([(20,20),(W-20,H-20)],radius=22,fill=(*CARD,255),outline=(*ACCENT,120),width=2)
    draw.rounded_rectangle([(20,20),(26,H-20)],radius=4,fill=(*ACCENT,220))
    for i in range(12): draw.line([(44,20+i),(W-44,20+i)],fill=(*ACCENT,int(60*(1-i/12))))
    ...
    if not pfp_placed:
        ...
        lw=int(draw.textlength(letter,font=f44))
        draw.text((AX-lw//2,AY-22),letter,fill=ACCENT,font=f44)

    draw.text((168,63),first_name.upper()[:16],fill=WHITE,font=f56)
    draw.text((170,130),f"@{username}",fill=GRAY,font=f22)

    def ordinal(n):
        s={1:"ST",2:"ND",3:"RD"}
        return f"{n}{s.get(n if n<20 else n%10,'TH')}"
    badge_text=f"{ordinal(level)} RANK"
    btw=int(draw.textlength(badge_text,font=f22)); pad=16
    bx1=W-44-btw-pad*2; by1,by2=36,82
    draw.rounded_rectangle([(bx1,by1),(W-44,by2)],radius=10,fill=GOLD)
    tw=int(draw.textlength(badge_text,font=f22))
    cx=bx1+((W-44-bx1)-tw)//2; cy=by1+((by2-by1)-22)//2
    ...
    for i,(lbl,val,unit) in enumerate(stats_d):
        bx=48+i*(box_w+10)
        draw.rounded_rectangle([(bx,box_y1),(bx+box_w,box_y2)],radius=14,fill=(22,24,40,255),outline=(*ACCENT,40),width=1)
        draw.rounded_rectangle([(bx,box_y1),(bx+box_w,box_y1+3)],radius=2,fill=(*ACCENT,120))
        draw.text((bx+12,box_y1+12),lbl,fill=LGRAY,font=f13)
        fv=f36 if draw.textlength(val,font=f36)<box_w-24 else f28
        val_y=box_y1+(box_y2-box_y1)//2-20
        draw.text((bx+12,val_y),val,fill=ACCENT2,font=fv)
        draw.text((bx+12,box_y2-20),unit,fill=LGRAY,font=f13)
    ...
    for r2 in range(8,0,-2):
        draw.ellipse([(bar_x1+fw-r2,bar_y+5-r2),(bar_x1+fw+r2,bar_y+5+r2)],fill=(*ACCENT2,int(60*(1-r2/8))))
    ...
    for i in range(10):
        bx3=48+i*30
        draw.rounded_rectangle([(bx3,418),(bx3+24,428)],radius=3,fill=ts_col if i<trust//10 else (36,38,58))
