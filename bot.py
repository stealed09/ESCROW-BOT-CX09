...
def detect_form_type(text: str) -> str | None:
    ...
    if any(k in norm for k in ["bet type", "bet deal", "bet amount", "party 1", "party 2", "game name"]):
        return "BET"
    if any(k in norm for k in ["third party", "third party username", "third party role", "third party charges"]):
        return "THIRD_PARTY"
    if any(k in norm for k in ["service type", "service deal", "work completion time", "proof of work"]):
        return "SERVICE"
    if any(k in norm for k in ["deal of", "total amount", "buyer username", "seller username", "buyer bank", "maximum time"]):
        return "NORMAL"
    return None
...
async def handle_both_agree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lock_msg += (
        ...
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"📌 <b>Next Steps:</b>\n"
        f"<code>/pay UPI_NAME</code> — send payment QR (raw)\n"
        f"<code>/pay FEE% UPI_NAME</code> — with fee added\n"
        f"<code>/add AMOUNT</code> — confirm received\n"
        f"<code>/close AMOUNT</code> — mark deal complete\n"
        f"<code>/transfer @admin</code> — transfer deal\n\n"
        f"<i>Reply to this message or use Trade ID with commands.</i>"
    )
...
async def cmd_form1(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "● Terms & Conditions :-\n\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "📌 Copy, fill, send in group.\n"
        "Admin reply 'BOTH AGREE' to lock.",
        parse_mode="HTML"
    )
...
async def cmd_formtype(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📋 <b>FORM TYPES</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━\n\n"
        "🔹 <b>form</b> — Normal Deal\n"
        "Regular buyer-seller escrow.\n"
        "Buyer pays, seller delivers.\n\n"
        "🎯 <b>form2</b> — Bet Deal\n"
        "Two parties bet on a game/event.\n"
        "Winner gets the pot.\n\n"
        "🤝 <b>form3</b> — Third Party Deal\n"
        "Three-way deal with a mediator.\n"
        "Third party verifies work.\n\n"
        "🛠️ <b>form4</b> — Service Deal\n"
        "Service provider delivers work.\n"
        "Payment released after proof.\n\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "Use <code>/form</code>, <code>/form2</code>, <code>/form3</code>, <code>/form4</code> to get blank forms.",
        parse_mode="HTML"
    )
...
async def ge_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Secondary group handler — detects BOTH AGREE on forms."""
    msg  = update.message
    user = update.effective_user
    if not msg or not msg.text:
        return
