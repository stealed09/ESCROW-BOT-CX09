...
async def _generate_profile_card(
    ...
    RANK_COLORS = {
        ...
        "GOLD":     (255,200,60),  "PLATINUM":(100,210,255),
        "ELITE":    (180,100,255),
    }
    RANK_NEXT = {
    ...
    # avatar
    AX,AY,AR = 94,104,54
    for r in range(AR+16,AR+2,-2):
        draw.ellipse([(AX-r,AY-r),(AX+r,AY+r)], fill=(*ACCENT,int(80*(1-(r-AR)/16))))
    draw.ellipse([(AX-AR-3,AY-AR-3),(AX+AR+3,AY+AR+3)], outline=ACCENT, width=3)
    ...
    # rank badge
    bt=f"  {rank}  LVL {level}  "; bw2=draw.textlength(bt,font=f22); bx1=W-56-bw2
    draw.rounded_rectangle([(bx1-2,40),(W-44,84)], radius=10,
                            fill=(*rank_col,25), outline=(*rank_col,160), width=2)
    draw.text((bx1+6,50), bt.strip(), fill=rank_col, font=f22)

    draw.line([(48,180),(W-48,180)], fill=(*ACCENT,40), width=1)

    # stat boxes
    stats_d=[("TOTAL VOLUME",f"₹{vol:,.0f}","INR"),
             ("DEALS DONE",str(deals),"COMPLETED"),
             ("HIGHEST DEAL",f"₹{highest:,.0f}","SINGLE")]
    ...
    for i,(lbl,val,unit) in enumerate(stats_d):
        ...
        fv=f32 if draw.textlength(val,font=f32)<box_w-24 else f22
        draw.text((bx+12,box_y1+30), val, fill=ACCENT2, font=fv)
    ...
    draw.text(((W-wmw)//2,466), wm, fill=(*ACCENT,50), font=f13)
