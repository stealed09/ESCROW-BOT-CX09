...
async def handle_new_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Send welcome when someone joins — GE group or P2P deal group."""
    chat    = update.effective_chat
    ...
    for member in members:
        ...
        # ── Check if this is a GE (group escrow) group ──
        ge_group = False
        if hasattr(state, "escrow_group_id") and state.escrow_group_id == chat.id:
            ge_group = True
        # Also check _ge_config
        try:
            if chat.id in _ge_config:
                ge_group = True
        except Exception:
            pass
...
def parse_field(text: str, *keys) -> str:
    """Extract value for a field from form text using multiple key variants."""
    ...
    for line in lines:
        ...
        for key in keys:
            ...
            if kn in nl:
                parts = line.split(":", 1)
                if len(parts) > 1:
                    return strip_field_value(parts[1])
                # Check for "- value" pattern
                parts2 = line.split("-", 1)
                if len(parts2) > 1:
                    return strip_field_value(parts2[1])
    return ""
...
async def handle_both_agree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Detect 'BOTH AGREE' reply from admin on a form message."""
    chat = update.effective_chat
    user = update.effective_user
    msg  = update.message
    ...
    # Check if this message is already locked
    rmid = replied_msg.message_id
    if rmid in _ge_msg_to_deal:
    ...
    buyer_raw  = form_data.get("buyer", "").strip()
    seller_raw = form_data.get("seller", "").strip()
    amount_raw = form_data.get("amount", "").strip()
    blank_fields = []
    if not buyer_raw or buyer_raw in ("—", "-", ""):
        blank_fields.append("● Buyer Username")
    if not seller_raw or seller_raw in ("—", "-", ""):
        blank_fields.append("● Seller Username")
    if not amount_raw or amount_raw in ("—", "-", ""):
        blank_fields.append("● Total Amount")
    if form_type == "NORMAL":
        if not form_data.get("deal_of","").strip() or form_data.get("deal_of","") in ("—","-",""):
            blank_fields.append("● Deal Of")
    if form_type == "BET":
        if not form_data.get("game_name","").strip() or form_data.get("game_name","") in ("—","-",""):
            blank_fields.append("● Game Name")

    if blank_fields:
        buyer_tag  = f"@{buyer_raw.lstrip('@')}"  if buyer_raw  not in ("—","-","") else ""
        seller_tag = f"@{seller_raw.lstrip('@')}" if seller_raw not in ("—","-","") else ""
