...
async def _generate_profile_card(
    ...
    try:
        ...
        f56=f44=f36=f32=f28=f22=f18=f15=f13=ImageFont.load_default()
    ...
    if not pfp_placed:
        ...
        # center letter in circle: x = AX - lw/2, y = AY - 22 (half of f44 height)
        draw.text((AX - lw//2, AY - 22), letter, fill=ACCENT, font=f44)

    # name + username — vertically centered with avatar (AY=104, AR=54 → top=50, bot=158)
    name_text = first_name.upper()[:16]
    # avatar center Y=104, name block = f56(~56px) + gap(8) + f18(~18px) = 82px total
    # center it: start_y = AY - 82//2 = 104 - 41 = 63
    name_y = 63
    draw.text((178, name_y), name_text, fill=WHITE, font=f56)
    draw.text((180, name_y + 62), f"@{username}", fill=GRAY, font=f22)
    ...
    for i,(lbl,val,unit) in enumerate(stats_d):
        ...
        # pick font size that fits box width
        fv = f36 if draw.textlength(val, font=f36) < box_w-24 else f28
        # vertically center value in box
