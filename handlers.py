...
# ─────────────────────────────────────────────
# /releaseto — Admin forced release
# ─────────────────────────────────────────────
...
async def cmd_releaseto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if deal.get("status") not in ["DISPUTED", "FUNDED"]:
        await update.message.reply_text(f"⚠️ Deal status is {deal.get('status')}. Cannot force release.")
        return

    if party not in ["buyer", "seller"]:
        await update.message.reply_text("❌ Specify 'buyer' or 'seller'.")
        return

    deal["status"] = "COMPLETED"
    deal["force_released_to"] = party
    deal["released_by_admin"] = user.username
    deal["completed_at"] = datetime.utcnow().isoformat()

    release_to_user = deal.get(f"{party}_username", "N/A")
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"Admin @{user.username} has released funds to the <b>{party.upper()}</b>.\n\n"
                f"✅ Released to: @{release_to_user}\n"
                f"🆔 Trade ID: <code>{deal_id}</code>\n\n"
                f"Deal is now closed."
            ),
            parse_mode="HTML"
        )
    except:
    ...
    await update.message.reply_text(
        f"✅ Funds released to {party} (@{release_to_user}) for deal {deal_id}."
    )

    await send_log(context,
        f"⚖️ <b>ADMIN FORCE RELEASE</b>\n\n"
        f"🆔 Trade ID: <code>{deal_id}</code>\n"
        f"⚖️ Released to: {party.upper()} (@{release_to_user})\n"
        f"👨‍💼 Admin: @{user.username}\n"
        f"📊 Status: COMPLETED (Force)\n"
        f"⏰ Time: {deal['completed_at']}"
    )
