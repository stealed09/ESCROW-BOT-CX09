...
async def cmd_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/profile or /myprofile - Show escrow stats"""
    user = update.effective_user
    uname = (user.username or "").lower()
    ...
    rank_emoji = {"Elite": "👑", "Platinum": "💎", "Gold": "🏆", "Silver": "🥈", "Bronze": "🥉"}.get(rank, "⭐")
    
    await update.message.reply_text(
        f"{rank_emoji} <b>ESCROW PROFILE</b>\n\n"
        f"👤 @{user.username}\n"
        f"━━━━━━━━━━━━━━\n"
        f"🏆 {rank} · Level {level}\n"
        f"💰 ₹{vol:,.0f}\n"
        f"📈 {stats['deals']} deals\n"
        f"🎯 Highest: ₹{stats['highest_deal']:,.0f}\n"
        f"━━━━━━━━━━━━━━\n\n"
        f"Keep trading to level up!",
        parse_mode="HTML"
    )
