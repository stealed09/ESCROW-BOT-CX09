...
async def idle_delete_loop(ctx, deal: dict, hours: int = 7):
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb_rows = [
        [InlineKeyboardButton("🔄 P2P ESCROW",    callback_data="deal_type:p2p"),
         InlineKeyboardButton("🛒 PRODUCT DEAL",  callback_data="deal_type:product")],
        [InlineKeyboardButton("📖 Instructions",   callback_data="show_instructions")],
    ]
    # Channel join button — agar admin ne set kiya hai
    if state.channel_link:
        kb_rows.insert(0, [InlineKeyboardButton(
            f"📢 {state.channel_name}",
            url=state.channel_link
        )])
    if is_admin(user.id):
        kb_rows.append([InlineKeyboardButton("👑 Admin Panel", callback_data="adm:status")])
    ...
    if is_admin(user.id):
        admin_cmds = (
            ...
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "<b>Deal Control:</b>\n"
            "<code>/adminrelease buyer TRD-XXXXXXXX</code>\n"
            "<code>/adminrelease seller TRD-XXXXXXXX</code>\n"
            "<code>/admindeposit TRD-XXXXXXXX</code>\n"
            "<code>/adminforcedeposit ADDRESS AMOUNT TOKEN</code>\n\n"
            "<b>Refund & Dispute:</b>\n"
            "<code>/refund TRD-XXXXXXXX</code>\n"
            "<code>/disputeend TRD-XXXXXXXX</code>\n"
            "<code>/canceldeal TRD-XXXXXXXX</code>\n\n"
            "<b>Setup:</b>\n"
            "<code>/setloggroup</code> — set log group\n"
            "<code>/setdisputegroup</code> — set dispute group\n"
            "<code>/setvouchgroup</code> — set vouch group\n"
            "<code>/setfee PERCENT</code>\n"
            "<code>/setbio TAG</code>  /setbiodiscount PERCENT\n"
            "<code>/setoxapay KEY</code>  /checkoxapay\n\n"
            "<b>Admins:</b>\n"
            "<code>/addadmin USER_ID</code>  /removeadmin  /listadmins\n\n"
            "<b>Info:</b>\n"
            "<code>/status</code>  /dealinfo TRD-XXXXXXXX  /adminpanel"
        )
...
async def handle_deal_type(update: Update, ctx: ContextTypes.DEFAULT_TYPE, d: str):
    ...
    bio_line     = f"\n🏷 Bio Discount: Add <code>{bio_tag}</code> to bio → <b>{bio_disc}%</b> fee!" if bio_tag else ""
    ...
    try:
        await ctx.bot.send_message(chat_id=group_id,
            text=(
                ...
                f"⌛ 7hrs no activity → group auto-deleted"),
            parse_mode="HTML")
    except Exception as e:
        logger.warning(f"Welcome msg error: {e}")

    await log_deal(ctx, deal, "Deal created")
    asyncio.create_task(idle_delete_loop(ctx, deal, hours=7))
    ...
    for member in members:
        ...
        if is_ge_group:
            welcome = (
                f"👋 𝗪𝗘𝗟𝗖𝗢𝗠𝗘 {uname}!\n\n"
                f"━━━━━━━━━━━━━━━━━━━\n\n"
                f"🔐 Aap <b>Baba Escrow</b> group mein aa gaye hain.\n\n"
                f"📋 <b>Deal karne ke liye forms:</b>\n\n"
                f"• <code>form</code>   — Normal Deal\n"
                f"• <code>form2</code>  — Bet Deal\n"
                f"• <code>form3</code>  — Third Party Deal\n"
                f"• <code>form4</code>  — Service Deal\n"
                f"• <code>formtype</code> — Sabka details\n\n"
                f"━━━━━━━━━━━━━━━━━━━\n\n"
                f"📌 <b>Deal flow:</b>\n"
                f"1️⃣ Form copy karo, fill karo, group mein bhejo\n"
                f"2️⃣ Admin <b>BOTH AGREE</b> reply karega → deal lock\n"
                f"3️⃣ Payment karo admin ke UPI pe\n"
                f"4️⃣ Admin <b>release</b> karega → confirm karo\n\n"
                f"━━━━━━━━━━━━━━━━━━━\n"
                f"⚠️ Koi bhi field blank mat chodo form mein!"
            )
        else:
            fee_pct = state.fee_percent
            welcome = (
...
def parse_field(text: str, *keys) -> str:
    """Extract value for a field from form text. Keys matched on normalized text, value from original."""
    ...
    for i, norm_line in enumerate(norm_lines):
        ...
        for key in keys:
            ...
            if kn in nl:
                # Use original line for value extraction to preserve @ and other chars
                ...
                if ":" in orig_line:
                    ...
                    if cleaned:
                        return cleaned
                # Split on first dash if no colon
                elif "-" in orig_line:
                    val = orig_line.split("-", 1)[1]
                    cleaned = strip_field_value(val)
                    if cleaned:
...
async def handle_both_agree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if bio_tag:
        try:
            b_info = await ctx.bot.get_chat(f"@{buyer_uname}")
            bio_status_buyer  = "✅ Set" if b_info.bio and bio_tag.lower() in b_info.bio.lower() else "❌ Not Set"
        except Exception:
            bio_status_buyer = "❓ Can't check"
        try:
            s_info = await ctx.bot.get_chat(f"@{seller_uname}")
            bio_status_seller = "✅ Set" if s_info.bio and bio_tag.lower() in s_info.bio.lower() else "❌ Not Set"
        except Exception:
            bio_status_seller = "❓ Can't check"
    ...
    if form_type == "BET":
        lock_msg += f"🎮 Game: {deal.get('game_name','—')}\n"
    elif form_type == "THIRD_PARTY":
        lock_msg += f"🔸 Third Party: {deal.get('third_party','—')}\n"
    elif form_type == "SERVICE":
        lock_msg += f"⚙️ Service: {deal.get('service_type','—')}\n"

    lock_msg += (
        f"\n👨‍⚖️ Escrower: @{user.username or user.id}\n"
        f"⏰ {ist_t}"
        + bio_line +
        f"\n━━━━━━━━━━━━━━━━━━━\n"
        f"📌 <b>Admin Commands:</b>\n\n"
        f"<code>/pay {tid} UPI_NAME</code> — Payment QR\n"
        f"<code>/add {tid} AMOUNT</code> — Amount received\n"
        f"<code>/close {tid} AMOUNT</code> — Deal complete\n"
        f"<code>/transfer {tid} @admin</code> — Transfer deal\n\n"
        f"💬 <code>release</code> likhke buyer/seller se confirm lo"
    )
    ...
    agree_msg = (
        f"🤝 <b>DEAL CONFIRMATION</b>\n\n"
        f"🆔 <code>{tid}</code>\n\n"
        f"{buyer_tag} {seller_tag}\n\n"
        f"Dono parties neeche apna button dabao 👇\n\n"
        f"✅ Agree = Deal confirm\n"
        f"❌ Disagree = Form reject (20 sec mein delete)\n\n"
        f"⏳ Dono agree karne ke baad deal <b>ACTIVE</b> hogi."
    )

    await msg.reply_text(lock_msg, parse_mode="HTML")
    agree_sent = await ctx.bot.send_message(
        chat_id=chat.id,
        text=agree_msg,
        parse_mode="HTML",
...
async def handle_ge_disagree(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    countdown_msg = await ctx.bot.send_message(
        ...
        text=(
            ...
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 <code>{tid}</code>\n\n"
            f"👤 {disagree_tag} ({role.title()}) ne disagree kiya!\n\n"
            f"{buyer_tag} {seller_tag}\n\n"
            f"⚠️ Form 20 seconds mein delete hoga.\n"
            f"Agar galti hui to admin se contact karo.\n"
    ...
    if deal.get("buyer_agreed") and deal.get("seller_agreed"):
        ...
        await ctx.bot.send_message(
            ...
            text=(
                ...
                f"✅ {seller_tag} — Agreed\n\n"
                f"━━━━━━━━━━━━━━━━━━━\n"
                f"💸 Ab admin UPI pe payment karo.\n"
                f"Admin confirm karega aur deal process hogi."
            ),
            parse_mode="HTML"
        )
        # Notify admin
...
async def cmd_ge_pay(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            "Usage:\n"
            "<code>/pay UPI_NAME</code> — raw amount\n"
            "<code>/pay 2 UPI_NAME</code> — with 2% fee added",
            parse_mode="HTML"
        )
        return
    ...
    if upi_name not in methods:
        ...
        await update.message.reply_text(
            f"❌ UPI method <code>{upi_name}</code> not found.\n\nAvailable: {avail}",
            parse_mode="HTML"
        )
        return
    ...
    raw_amt_str = deal.get("amount", "0").replace("₹", "").replace(",", "").strip()
    try:
        raw_amt = float(_re.sub(r"[^\d.]", "", raw_amt_str))
    except Exception:
        raw_amt = 0.0
...
async def cmd_ge_close(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"⏰ {closed_time}\n\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"⭐ Dono parties apni rating do neeche!",
        parse_mode="HTML"
    )
    ...
    await ctx.bot.send_message(
        ...
        text=(
            f"⭐ <b>DEAL RATING</b>\n\n"
            f"🆔 <code>{tid}</code>\n\n"
            f"Deal khatam hua! Dono parties ki rating do 👇\n"
            f"Ye vouch group mein save hogi."
        ),
        parse_mode="HTML",
...
async def ge_payment_reminder(ctx, tid: str, chat_id: int):
    """Send payment reminder after 30 min, 1hr, 2hr."""
    deal = _ge_deals.get(tid, {})
    buyer_uname = deal.get("buyer_uname", deal.get("buyer", "")).lstrip("@")
    buyer_tag   = f"@{buyer_uname}" if buyer_uname else "Buyer"

    for mins, label in [(30, "30 minutes"), (60, "1 ghanta"), (120, "2 ghante")]:
        ...
        try:
            await ctx.bot.send_message(
                ...
                text=(
                    ...
                    f"🆔 <code>{tid}</code>\n\n"
                    f"👉 {buyer_tag}\n\n"
                    f"⚠️ {label} ho gaya — abhi tak payment confirm nahi hua!\n"
                    f"Jaldi karo warna deal auto-cancel ho sakti hai.\n\n"
                    f"💬 Payment ke baad admin ko batao."
                ),
                parse_mode="HTML"
            )
        except Exception:
...
async def cmd_calc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """/calc AMOUNT or just 'calc 5000' in group"""
    args = ctx.args or []
    if not args:
        await update.message.reply_text(
            f"💰 <b>Fee Calculator</b>\n\n"
            f"Usage: <code>calc 5000</code>\n"
            f"Ya: <code>/calc 5000</code>\n\n"
            f"Current fee: <b>{state.fee_percent}%</b>",
            parse_mode="HTML"
        )
        return
    try:
        amount = float(_re.sub(r"[^\d.]", "", args[0]))
        if amount <= 0:
            raise ValueError
    except Exception:
        await update.message.reply_text("❌ Valid amount do. Example: <code>calc 5000</code>", parse_mode="HTML")
        return
    ...
    text = (
        ...
        f"💸 Total Payable: <b>₹{total:,.2f}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━"
    )
    if bio_tag and bio_disc > 0:
        disc_fee  = round(amount * bio_disc / 100, 2)
        disc_total = round(amount + disc_fee, 2)
        text += (
            f"\n\n🏷 <b>Bio Discount</b> (<code>{bio_tag}</code>):\n"
            f"📊 Fee ({bio_disc}%): <b>₹{disc_fee:,.2f}</b>\n"
            f"💸 Total Payable: <b>₹{disc_total:,.2f}</b>\n\n"
            f"📌 Bio mein <code>{bio_tag}</code> add karo discount ke liye!"
        )
    await update.message.reply_text(text, parse_mode="HTML")
...
async def _ge_handle_release(update: Update, ctx: ContextTypes.DEFAULT_TYPE, deal: dict):
    """Admin ne 'release' likha — buyer/seller ko tag karo, confirm maango."""
    chat   = update.effective_chat
    user   = update.effective_user
    tid    = deal.get("tid", "—")
    buyer_uname  = deal.get("buyer_uname",  deal.get("buyer",  "—")).lstrip("@")
    ...
    await ctx.bot.send_message(
        chat_id=chat.id,
        text=(
            f"💸 <b>RELEASE REQUEST</b>\n\n"
            f"━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 Trade ID: <code>{tid}</code>\n"
            f"💰 Amount: <b>{amt}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━\n\n"
            f"👉 {buyer_tag} {seller_tag}\n\n"
            f"Admin amount release karne wala hai.\n"
            f"<b>Apna button dabao confirm karne ke liye 👇</b>\n\n"
            f"⚠️ Agree na karne par admin se baat karo.\n"
            f"Admin ko directly contact karo agar koi issue hai."
        ),
        parse_mode="HTML",
        reply_markup=kb
    )

    # Schedule reminder after 5 minutes if no response
    async def release_reminder():
        ...
        if d.get("release_buyer_agreed") and d.get("release_seller_agreed"):
            return  # Already done
        pending = []
        if not d.get("release_buyer_agreed"):
            pending.append(buyer_tag)
        if not d.get("release_seller_agreed"):
            pending.append(seller_tag)
        if pending:
            try:
                await ctx.bot.send_message(
                    chat_id=chat.id,
                    text=(
                        f"⏰ <b>RELEASE REMINDER</b>\n\n"
                        f"🆔 <code>{tid}</code>\n\n"
                        f"{'  '.join(pending)} — abhi tak confirm nahi kiya!\n\n"
                        f"⚠️ Agar koi issue hai to admin se baat karo.\n"
                        f"Admin ke bina release nahi hogi."
                    ),
                    parse_mode="HTML"
                )
            except Exception:
    ...
    asyncio.create_task(release_reminder())
...
async def ge_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Secondary group handler — form triggers, BOTH AGREE, release trigger."""
    msg  = update.message
    user = update.effective_user
    chat = update.effective_chat
    if not msg or not msg.text:
    ...
    # ── Group restriction — sirf escrow group mein kaam karo ──
    # Agar escrow_group_id set hai to sirf wahi, warna sab groups mein kaam karo
    if getattr(state, "escrow_group_id", None) and chat.id != state.escrow_group_id:
        return

    text = msg.text.strip()

    # ── "me" ko sender ke @username se replace karo ──
    sender_tag = f"@{user.username}" if user.username else user.first_name
    import re as _re
    # Word boundary se "me" replace karo — case insensitive
    text = _re.sub(r'(?<![a-zA-Z@])me(?![a-zA-Z])', sender_tag, text, flags=_re.IGNORECASE)

    norm = normalize_text(text).upper()
    text_lower = text.lower().strip()

    # ── Plain text form triggers (no slash needed) ──
    if text_lower == "form":
        await update.message.reply_text(FORM1_TEXT)
        return
    elif text_lower == "form2":
        await update.message.reply_text(FORM2_TEXT)
        return
    elif text_lower == "form3":
        await update.message.reply_text(FORM3_TEXT)
        return
    elif text_lower == "form4":
        await update.message.reply_text(FORM4_TEXT)
        return
    elif text_lower == "formtype":
        await cmd_formtype(update, ctx)
        return

    # ── calc trigger — "calc 5000" ──
    elif text_lower.startswith("calc ") or text_lower == "calc":
        parts = text.split(None, 1)
        ctx.args = [parts[1]] if len(parts) > 1 else []
        await cmd_calc(update, ctx)
        return

    # ── Admin "release" text/command trigger ──
    is_release = (
        text_lower == "release" or
        text_lower.startswith("release ") or
        text_lower.startswith("/release")
    )
    if is_release and is_admin(user.id):
        # TID from text? e.g. "release GE-XXXXXXXX" or "/release GE-XXXXXXXX"
        parts = text.split()
        tid_arg = None
        for p in parts[1:]:
            if p.upper().startswith("GE-"):
                tid_arg = p.upper()
                break
        ...
        if not deal:
            deal = _resolve_ge_deal(update)
        if not deal:
            # Find latest deal in this group — LOCKED or ACTIVE
            for d in reversed(list(_ge_deals.values())):
                if d.get("group_id") == chat.id and d.get("status") in ("LOCKED", "ACTIVE"):
                    deal = d
                    break
        ...
        if deal.get("status") not in ("LOCKED", "ACTIVE"):
            await msg.reply_text(
                f"❌ Deal <code>{deal.get('tid')}</code> already "
                f"{deal.get('status')} hai.",
                parse_mode="HTML"
            )
            return
    ...
    # ── Track latest form + IMMEDIATE blank field check ──
    if detect_form_type(text):
        # Store the "me"-replaced text so parse_form gets correct usernames
        _ge_group_latest_form[update.effective_chat.id] = {
            "text": text,  # already has "me" replaced above
            "message_id": msg.message_id
        }
        # Turant check karo — form bhejne ke baad hi bolo blank fields
        form_type = detect_form_type(text)
        form_data = parse_form(text, form_type)
        ...
        if is_blank(amount_raw):
            blank_fields.append("● Total Amount")
        if form_type == "NORMAL":
            if is_blank(form_data.get("deal_of", "")):
                blank_fields.append("● Deal Of")
            if is_blank(form_data.get("max_time", "")):
                blank_fields.append("● Maximum Time")
            if is_blank(form_data.get("payment_method", "")):
                blank_fields.append("● Buyer Bank Name / Payment Method")
            if is_blank(form_data.get("terms", "")):
                blank_fields.append("● Terms & Conditions")
        elif form_type == "BET":
            if is_blank(form_data.get("game_name", "")):
                blank_fields.append("● Game Name")
            if is_blank(form_data.get("bet_type", "")):
                blank_fields.append("● Bet Type")
            if is_blank(form_data.get("p1_loss", "")):
                blank_fields.append("● Party 1 ka loss condition")
            if is_blank(form_data.get("p2_loss", "")):
                blank_fields.append("● Party 2 ka loss condition")
            if is_blank(form_data.get("max_time", "")):
                blank_fields.append("● Maximum Time")
        elif form_type == "THIRD_PARTY":
            if is_blank(form_data.get("deal_of", "")):
                blank_fields.append("● Deal Of")
            if is_blank(form_data.get("third_party", "")):
                blank_fields.append("● Third Party Username")
            if is_blank(form_data.get("tp_role", "")):
                blank_fields.append("● Third Party Role")
            if is_blank(form_data.get("max_time", "")):
                blank_fields.append("● Maximum Time")
            if is_blank(form_data.get("terms", "")):
                blank_fields.append("● Terms & Conditions")
        elif form_type == "SERVICE":
            if is_blank(form_data.get("service_type", "")):
                blank_fields.append("● Service Type")
            if is_blank(form_data.get("work_details", "")):
                blank_fields.append("● Work Details")
            if is_blank(form_data.get("completion_time", "")):
                blank_fields.append("● Work Completion Time")
            if is_blank(form_data.get("proof_of_work", "")):
                blank_fields.append("● Proof of Work")
            if is_blank(form_data.get("terms", "")):
                blank_fields.append("● Terms & Conditions")
        ...
        if blank_fields:
            ...
            await msg.reply_text(
                f"⚠️ <b>FORM INCOMPLETE!</b>\n\n"
                f"{tags if tags else '👆 Form bhejne wale'}\n\n"
                f"Ye fields khali hain:\n\n"
                f"{fields_text}\n\n"
                f"Sab fill karke dobara form bhejo.\n"
                f"Tab admin <b>BOTH AGREE</b> karega. ✅",
                parse_mode="HTML"
            )
            return
