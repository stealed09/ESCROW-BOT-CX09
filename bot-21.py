...
async def create_group_telethon(title: str, bot_username: str):
    ...
    try:
        ...
        # Kick the Telethon account (creator) from the group
        # so only bot + invited parties remain
        try:
            ...
            logger.warning(f"Could not kick creator from group: {e}")
...
async def handle_start_deal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await ctx.bot.send_message(
        ...
        text=(
            ...
            f"➡️ <b>Next step:</b> Both join the group, then anyone sends <b>/dd</b> to open the deal form."
        ),
        parse_mode="HTML"
    )
    ...
    try:
        await ctx.bot.send_message(
            ...
            text=(
                ...
                f"🆔 Trade ID: <code>{tid}</code>\n\n"
                f"Both buyer and seller must join.\n\n"
                f"➡️ <b>Next step:</b> Either party sends <b>/dd</b> to start filling the deal form."
            ),
            parse_mode="HTML"
        )
    except Exception as e:
...
async def group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id in _address_waiting:
        ...
        if b and s:
            ...
            next_step = "⏳ Waiting for seller to send <b>/seller</b>"
        else:
            next_step = "⏳ Waiting for buyer to send <b>/buyer</b>"

        label = "🛒 Buyer" if role == "buyer" else "🏪 Seller"
        await msg.reply_text(
            f"✅ <b>{label} Registered!</b>\n\n"
...
async def _set_role_prompt(update: Update, ctx: ContextTypes.DEFAULT_TYPE, role: str):
    ...
    if deal["status"] not in ("SETUP", "ROLES_SET"):
        await update.message.reply_text(
            f"⚠️ Cannot change roles. Status: <b>{deal['status']}</b>",
            parse_mode="HTML"
        )
        return
    ...
    # Prevent overwriting if already set by someone else
    existing_id = deal.get(f"{role}_id")
    if existing_id and existing_id != user.id:
        await update.message.reply_text(
            f"❌ {role.capitalize()} is already set by @{deal.get(f'{role}_username')}.",
            parse_mode="HTML"
        )
        return
    ...
    await update.message.reply_text(
        ...
        f"<i>Your next message will be captured as your address.</i>",
        parse_mode="HTML"
    )
...
async def cmd_release(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🛒 Buyer Address: <code>{deal.get('buyer_address', 'N/A')}</code>\n\n"
            ...
            try:
                await ctx.bot.send_message(
                    ...
                    text=(
                        ...
                        f"Released: <b>{final:.4f} {deal.get('token')}</b>\n"
                        f"📬 Buyer's address: <code>{buyer_addr}</code>\n\n"
...
async def handle_dispute_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE, d: str):
    """Admin clicks 'Add Me to Group' — generates 1-use invite link."""
    q = update.callback_query
    user = q.from_user
    did = d.split(":", 1)[1]
    ...
    await q.edit_message_text(
        ...
        f"<code>/adminrelease buyer {did}</code>\n"
        f"<code>/adminrelease seller {did}</code>\n"
        f"<code>/refund {did}</code>\n"
        f"<code>/disputeend {did}</code>",
        parse_mode="HTML"
    )
